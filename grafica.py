...
def grafica(x, xn0, h, hn0, y, yn0):
    ...
    if max(y) < 20: 
        ...
        plt.yticks(np.arange(min(y), max(y)+1, step=5))
    elif max(y) < 60:
        plt.yticks(np.arange(min(y), max(y)+1, step=10))
    elif max(y) < 80:
        plt.yticks(np.arange(min(y), max(y)+1, step=15))
    elif max(y) < 100:
        plt.yticks(np.arange(min(y), max(y)+1, step=20))
